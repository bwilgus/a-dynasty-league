...
- **`player_id` in `lineups`/`draft_picks` is normalized to Sleeper's ID space** (2025-09 session, `normalize_player_ids.py`). Before this, ESPN-era (2017–2023) and NFL.com-era (2014–2016) rows used those platforms' own IDs — e.g. Tom Brady was `2330` every ESPN year, a different value from his Sleeper ID `167`. Both tables now have `espn_player_id` (the original pre-normalization id, always preserved) and `player_id_source` (`'sleeper'` if `player_id` now holds a confirmed Sleeper-space id, `'unmatched'` if no confident match was found and `player_id` still holds the original value). D/ST rows are handled separately from name matching: both eras spell defenses as a team name, and Sleeper's own D/ST "id" is just the team abbreviation (e.g. `GB`), so it's a deterministic nickname→abbreviation lookup (`NICKNAME_TO_ABBR` in the script) rather than fuzzy matching. **Only trust `player_id` for cross-era joins where `player_id_source = 'sleeper'`** — for `'unmatched'` rows it's still whatever platform-native id it always was.
  - Matching is name+position against a live fetch of Sleeper's `/players/nfl` endpoint, falling back to name-only (if exactly one Sleeper player has that name) and a small hand-verified alias table for confirmed nickname/spelling mismatches (`VERIFIED_ALIASES` in the script, e.g. "Pat Mahomes" -> Sleeper's "Patrick Mahomes"). It never guesses when ambiguous — real duplicate identities (e.g. two different NFL players named "Mike Williams", "Frank Gore", or "Zach Miller" across history) are deliberately left as `'unmatched'` rather than risk assigning the wrong one.
  - Coverage as of last run: ~99% of `lineups` and ~97% of `draft_picks` distinct (name, position) pairs matched. Remaining `'unmatched'` rows are a mix of: players no longer present in Sleeper's *current* `/players/nfl` snapshot (it's a live endpoint, not a stable historical archive — e.g. Davante Parker was absent as of this run despite being a real, well-known player), genuine name-collision ambiguity, and a few rows in `draft_picks` with corrupted source text predating this work (e.g. one name field that's a garbled concatenation of two different players' names, and stray `\xa0` characters in some `position` values) — those are pre-existing data-quality issues in `draft_picks`, not a normalization bug.
